  ...
  <TooltipProvider>
    <Tooltip>
      <TooltipTrigger asChild>
        <Link href={`plans/${plan.id}`} className="duration-300">
          <Card className="hover:bg-slate-50 dark:hover:bg-slate-900">
            <CardHeader>
              <CardTitle>{plan.title}</CardTitle>
              <CardDescription>
                Fecha de préstamo: {format(new Date(plan.loan.date), "PP")}
              </CardDescription>
            </CardHeader>
            <CardContent className="flex flex-col gap-y-5">
              <section className="flex justify-between gap-x-10">
                <div className="flex flex-col items-center">
                  <span className="text-gray-500 text-xs"> Monto total</span>
                  {formatCurrency(plan.stats.totalAmount, plan.loan.currency)}
                </div>
                <div className="flex flex-col items-center">
                  <span className="text-gray-500 text-xs"> Cuota inicial</span>
                  {formatCurrency(plan.stats.downPayment, plan.loan.currency)}
                </div>
                <div className="flex flex-col items-center">
                  <span className="text-red-500 text-xs"> Monto restante</span>
                  {formatCurrency(
              ...
              <section className="flex gap-x-3 justify-between">
                ...
                <div className="flex flex-col items-center">
                  <span className="text-gray-500 text-xs">Cuota mensual</span>
                  {formatCurrency(
